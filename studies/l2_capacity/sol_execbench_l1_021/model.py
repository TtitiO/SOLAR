"""021_vision_cu_seqlens_variable_length_attention (sol_execbench_l1_021) — optimized kernel.

DSL chosen: PyTorch — reason: native PyTorch for SOLAR traceability
Operator: Vision encoder attention with variable-length sequence support using cumulative sequence lengths (cu_seqlens). Processes multiple images/videos with d
"""
...
class Model(nn.Module):
    ...
    def forward(self, hidden_states, cu_seqlens, cos, sin, qkv_weight, qkv_bias, proj_weight, proj_bias):
        return run(hidden_states,cu_seqlens,cos,sin,qkv_weight,qkv_bias,proj_weight,proj_bias)


def get_inputs():
    """Return concrete benchmark inputs on CPU."""
    torch.manual_seed(0)
    axes = {'num_seqs': 2048, 'total_seq_len': 2048}

"""021_cross_attention_text_video_conditioning_backward (sol_execbench_l2_021) — optimized kernel.

DSL chosen: PyTorch — reason: native PyTorch for SOLAR traceability
Operator: Backward pass for cross-attention text-video conditioning. Computes gradients through output projection, attention mechanism (softmax, Q@K^T), and Q/K
"""
...
class Model(nn.Module):
    """Thin wrapper: forward() calls run()."""
    def __init__(self):
        super().__init__()

    def forward(self, grad_output, video_latents, text_embeddings, query_weight, query_bias, key_weight, key_bias, value_weight, value_bias, output_weight, output_bias, scale):
        return run(grad_output,video_latents,text_embeddings,query_weight,query_bias,key_weight,key_bias,value_weight,value_bias,output_weight,output_bias,scale)


def get_inputs():
    """Return concrete benchmark inputs on CPU."""
    torch.manual_seed(0)
    grad_output = torch.randn(1, 2048, 1024, dtype=torch.float32)
    video_latents = torch.randn(1, 2048, 1024, dtype=torch.float32)
    text_embeddings = torch.randn(1, 2048, 1024, dtype=torch.float32)
    query_weight = torch.randn(1024, 1024, dtype=torch.float32)
    query_bias = torch.randn(1024, dtype=torch.float32)
    key_weight = torch.randn(1024, 1024, dtype=torch.float32)
    key_bias = torch.randn(1024, dtype=torch.float32)
    value_weight = torch.randn(1024, 1024, dtype=torch.float32)
    value_bias = torch.randn(1024, dtype=torch.float32)
    output_weight = torch.randn(1024, 1024, dtype=torch.float32)
    output_bias = torch.randn(1024, dtype=torch.float32)
    scale = 1.0
    return [grad_output, video_latents, text_embeddings, query_weight, query_bias, key_weight, key_bias, value_weight, value_bias, output_weight, output_bias, scale]

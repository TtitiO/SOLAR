"""072_region_aware_self_attention_with_edit_bias_backward (sol_execbench_l2_072) — optimized kernel.

DSL chosen: PyTorch — reason: native PyTorch for SOLAR traceability
Operator: Backward pass for region-aware self-attention with edit bias. Computes gradients through QKV projection, scaled dot-product attention with position-ba
"""
...
# ── Include verbatim reference code ──

import torch
import torch.nn.functional as F


def _get_inputs(axes_and_scalars: dict, device: torch.device) -> dict:
...
class Model(nn.Module):
    """Thin wrapper: forward() calls the reference run() function."""
    def __init__(self):
        super().__init__()

    def forward(self, grad_output, hidden_states, edit_region_mask, qkv_weight, qkv_bias, out_weight, out_bias, edit_region_bias, within_edit_bias, cross_edit_bias, attention_mask, query, key, value, attention_scores, attention_probs, attention_probs_dropped, attention_output, dropout_mask, scale, dropout_p):
        return run(grad_output, hidden_states, edit_region_mask, qkv_weight, qkv_bias, out_weight, out_bias, edit_region_bias, within_edit_bias, cross_edit_bias, attention_mask, query, key, value, attention_scores, attention_probs, attention_probs_dropped, attention_output, dropout_mask, scale, dropout_p)


def get_inputs():
    """Return concrete benchmark inputs on CPU."""
    torch.manual_seed(0)
    axes = {'batch_size': 1, 'seq_len': 2048}
    return list(_get_inputs(axes, torch.device('cpu')).values())

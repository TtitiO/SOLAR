"""015_grouped_query_attention_with_rope_and_qk_norm (sol_execbench_l1_015) — optimized kernel.

DSL chosen: PyTorch — reason: native PyTorch for SOLAR traceability
Operator: Grouped Query Attention with RoPE and Query/Key RMSNorm. Features 32 query heads, 8 KV heads (4x grouping), per-head RMSNorm on Q/K before RoPE applic
"""
...
class Model(nn.Module):
    ...
    def forward(self, hidden_states, cos, sin, attention_mask, q_proj_weight, k_proj_weight, v_proj_weight, o_proj_weight, q_norm_weight, k_norm_weight, rms_norm_eps, scaling):
        return run(hidden_states,cos,sin,attention_mask,q_proj_weight,k_proj_weight,v_proj_weight,o_proj_weight,q_norm_weight,k_norm_weight,rms_norm_eps,scaling)


def get_inputs():
    """Return concrete benchmark inputs on CPU."""
    torch.manual_seed(0)
    hidden_states = torch.randn(1, 2048, 4096, dtype=torch.bfloat16)
    cos = torch.randn(1, 2048, 128, dtype=torch.bfloat16)
    sin = torch.randn(1, 2048, 128, dtype=torch.bfloat16)
    attention_mask = torch.randn(1, 1, 2048, 2048, dtype=torch.bfloat16)
    q_proj_weight = torch.randn(4096, 4096, dtype=torch.bfloat16)
    k_proj_weight = torch.randn(1024, 4096, dtype=torch.bfloat16)
    v_proj_weight = torch.randn(1024, 4096, dtype=torch.bfloat16)
    o_proj_weight = torch.randn(4096, 4096, dtype=torch.bfloat16)
    q_norm_weight = torch.randn(128, dtype=torch.bfloat16)
    k_norm_weight = torch.randn(128, dtype=torch.bfloat16)
    rms_norm_eps = 1e-5
    scaling = 1.0
    return [hidden_states, cos, sin, attention_mask, q_proj_weight, k_proj_weight, v_proj_weight, o_proj_weight, q_norm_weight, k_norm_weight, rms_norm_eps, scaling]
